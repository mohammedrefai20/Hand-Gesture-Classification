...
# Start Parent Run
# =============================== 
mu.start_run(run_name="KNN_GridSearch")
...
for i, params in enumerate(grid_knn.cv_results_['params']):
    ...
    run_name = f"KNN/metric={params['metric']}/n_neighbors={params['n_neighbors']}/weights={params['weights']}_run_No.{i}"
    ...
    metrics = {
        "mean_cv_f1_score": grid_knn.cv_results_['mean_test_score'][i],
        "mean_train_score": grid_knn.cv_results_['mean_train_score'][i],
    }
...
# Log best parameters
mu.log_params(grid_knn.best_params_)

# Log best model
mu.log_sklearn_model(best_model, artifact_path="best_knn_model")
...
fig.suptitle("KNN Confusion Matrices Analysis", fontsize=16, y=0.98)

plt.savefig("artifacts/knn_confusion_matrices.png", dpi=300, bbox_inches="tight")
