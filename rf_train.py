...
# Start Parent Run
# =============================== 
mu.start_run(run_name="RandomForest_GridSearch")
...
for i, params in enumerate(grid_rf.cv_results_['params']):
    ...
    run_name = f"RF/n_est={params['n_estimators']}/depth={params['max_depth']}_run_No.{i}"
    ...
    metrics = {
        "mean_cv_f1_score": grid_rf.cv_results_['mean_test_score'][i],
        "mean_train_score": grid_rf.cv_results_['mean_train_score'][i],
    }
...
# Log best parameters
mu.log_params(grid_rf.best_params_)

# Log best model
mu.log_sklearn_model(best_model, artifact_path="best_rf_model")
...
fig.suptitle("Random Forest Confusion Matrices Analysis", fontsize=16, y=0.98)
plt.savefig("artifacts/rf_confusion_matrices.png", dpi=300, bbox_inches="tight")
plt.close()
